...
function renderFox() {
  ...
  elements.foxFeature.innerHTML = items[0]
    ...
    : emptyState("Fox", foxSource?.display_url, "Fox-videor ar inte cachelagrade an.")
        ...
        <section class="stream-section stream-section--newsroom" id="provider-${escapeHtml(slugify(provider))}" style="--section-accent:${escapeHtml(accent)}">
          ...
          ${
            items[0]
              ...
              : emptyState(provider, activeSource?.display_url, `${provider} ar inte cachelagrad an.`)
          }
        </section>
      `
    })
...
function renderLive() {
  ...
  elements.liveFeature.innerHTML = items[0]
    ...
    : emptyState("Live", sourceById("expressen-direkt")?.display_url, "Inga livefloden ar cachelagrade an.")
